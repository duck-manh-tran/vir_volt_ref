...
def CTAT_var_anls(lib, dsnwk):
	...
	def variable_anls(n_vals):
		...
		return diffs, divs, y_vals, ey_vals

	
	n_vals_vdd_8 = data_as_vdd[4, :, 3:6] * 1e6		# n_vals with vdd = 0.8
	temps = data_as_vdd[4, :, 0] 	
	r_caps = data_as_vdd[4, : 2] 	
	diffs, divs, y_vals, ey_vals = variable_anls(n_vals_vdd_8) 
	
	list_obj = []
	fig2 = plt.figure('temp_var_anls')
	n_rows = 2
	n_cols = 3
	colors = ['navy', 'blue', 'dodgerblue']
	...
	names = ['$n_1$', '$n_2$', '$n_3$']
	for idd in range(3):
		list_obj[0].scatter(temps, r_caps, n_vals.T[ix], c=colors[ix], label=names[ix])
